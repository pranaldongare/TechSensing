"""
China-focused prompt set for the China Focus report mode.

Kept FULLY SEPARATE from the general sensing prompts (rather than injecting a
directive into them) for reliability: when China Focus is ON the entire report
is China-scoped, and these prompts drive classification and all four report
phases.
...
# Shared China scope definition reused across prompts.
_CHINA_SCOPE = (
    "This report is EXCLUSIVELY about CHINA's developments in '{domain}'. "
    "'China' covers mainland Chinese companies, AI labs, universities and "
    "research institutes, open-source projects, investors, and government / "
    "policy bodies. Developments led by US, European, or other non-Chinese "
    "organizations are OUT OF SCOPE except where explicitly used for comparison."
)
...
# The four China streams used to frame topic categorization.
_STREAMS_BLOCK = (
...
def china_classify_prompt(
    ...
    """China-focused classification of a batch of articles."""
    people_block = ""
    if key_people:
    ...
    scope = _CHINA_SCOPE.format(domain=domain)

    contents = [
        {
            "role": "system",
            "parts": (
                f"You are a senior analyst specializing in CHINA's '{domain}' landscape.\n\n"
                ...
                f"{scope}\n"
                f"- An article is relevant ONLY if its PRIMARY subject is a CHINESE development "
                f"in '{domain}' (a Chinese company, lab, university, project, investor, or policy).\n"
                "- Articles primarily about non-Chinese organizations should receive relevance_score < 0.2, "
                "even if they mention China in passing.\n"
                ...
                "- Omit non-China or off-domain articles entirely.\n"
            ),
        },
        {
            "role": "user",
            "parts": (
                ...
                f"Classify each article that is DIRECTLY about CHINA's developments in '{domain}'. "
                "Exclude non-Chinese or off-domain articles. The articles array MUST contain "
    ...
    contents = [
        {
            ...
            "parts": (
                ...
                f"CHINA FOCUS report on the '{domain}' domain.\n\n"
                f"{scope}\n\n"
                "Generate the CORE of the report: report_title, bottom_line, executive_summary, "
                "topic_highlights, top_events, and key_trends — ALL about China's activity in "
                f"'{domain}'.\n\n"
                + _STREAMS_BLOCK
                + "SECTION GUIDELINES:\n"
                "- report_title: must make the China focus explicit, e.g. "
                f"\"China Focus: {domain} — {date_range}\".\n"
                "- bottom_line: 2-3 sentences — the single most important takeaway about China this period.\n"
                "- top_events: the TOP ~10 most impactful CHINESE events of the period, each with "
                "headline, actor (Chinese org/person), event_type, impact_summary, strategic_intent, "
                ...
                "**Why It Matters** / **What To Do**, framed for a reader tracking China's "
                f"'{domain}' ecosystem.\n"
                "- topic_highlights: 4-8 scannable China updates (short label + 1-2 sentences).\n"
                "- key_trends: 5-10 China-specific trends with evidence and why they matter.\n\n"
                "GROUNDING AND CITATION RULES:\n"
                "- Every claim MUST be grounded in the provided articles (all China-scoped).\n"
        ...
        },
        {
            ...
            "parts": (
                ...
                f"DOMAIN: {domain} (CHINA FOCUS)\n\n"
                f"CLASSIFIED CHINA ARTICLES:\n\n{classified_articles_json}\n\n"
                "Generate the China-focused report core: report_title, bottom_line, executive_summary, "
...
def china_report_radar_prompt(
    ...
    """Phase 2 (China): technology radar entries from the Chinese ecosystem."""
    scope = _CHINA_SCOPE.format(domain=domain)
    contents = [
        {
            "role": "system",
            "parts": (
                ...
                f"weekly CHINA FOCUS report on the '{domain}' domain.\n\n"
                f"{scope}\n\n"
                ...
                f"framework from CHINA's '{domain}' ecosystem.\n"
                ...
                "GROUNDING:\n"
                "- Ground every radar item in the provided China articles; do not fabricate.\n\n"
                + _custom_requirements_block(custom_requirements)
                + _OUTPUT_RULES
            ),
        },
        {
            ...
            "parts": (
                ...
                f"PHASE 1 CONTEXT:\n{core_context_json}\n\n"
                f"CLASSIFIED CHINA ARTICLES:\n\n{classified_articles_json}\n\n"
                "Generate radar_items for China's technologies in this domain. Return ONLY valid JSON."
            ),
        },
    ]
    return contents
...
def china_report_insights_prompt(
    classified_articles_json: str,
    core_context_json: str,
    ...
    """Phase 3 (China): market signals, report sections, recommendations, notable articles."""
    people_block = ""
    if key_people:
        names = ", ".join(key_people)
    ...
    contents = [
        {
            "role": "system",
            "parts": (
                "You are a senior technology strategist continuing a weekly CHINA FOCUS report on "
                f"the '{domain}' domain.\n\n"
                f"{scope}\n\n"
                ...
                "FRAMING:\n"
                "- All content is about China's activity in this domain.\n"
                "- recommendations are for a reader/organization TRACKING China — e.g. what to watch, "
                "evaluate, or respond to given these Chinese developments.\n\n"
                + _STREAMS_BLOCK
                + "GROUNDING:\n- Ground everything in the provided China articles; cite source_urls; do not fabricate.\n\n"
                + _custom_requirements_block(custom_requirements)
...
def china_details_prompt(
    ...
    """Phase 4 (China): detailed deep-dive write-ups for selected Chinese technologies."""
    scope = _CHINA_SCOPE.format(domain=domain)
    contents = [
        {
            "role": "system",
            "parts": (
                ...
                f"for a CHINA FOCUS report on the '{domain}' domain.\n\n"
                f"{scope}\n\n"
                + _custom_requirements_block(custom_requirements)
                + (
                ...
                "matters, maturity, practical applications) based on the China articles provided. "
