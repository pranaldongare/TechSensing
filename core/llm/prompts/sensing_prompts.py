    ...
    return [
        {
            ...
            "parts": (
                ...
                "1. card_title (<=80 chars): lead with the ORGANIZATION, then an action verb "
                "(Releases / Open-Sources / Launches / Updates / Unveils), then the EXACT product "
                "name with version/size code (e.g. '<Org> Releases <Model vX-size>').\n"
                ...
                "5. metrics: 0-5 boxed stats — hard numbers: sizes (total/active params), context "
                "length, throughput/speed, accuracy/benchmarks, cost, dates. Put the BASELINE and "
                "any TRADEOFF in 'comparison' (e.g. label='Throughput', value='~6x', "
                "comparison='vs comparable open LLMs, on-par accuracy'; or label='Medium mode', "
                "value='2.5x fewer tokens', comparison='at ~7% accuracy drop').\n"
                "6. bullets: 3-5 TELEGRAPHIC technical facts (each ONE fact, <=160 chars) — expert "
                "analyst shorthand, NOT full sentences, no marketing. Use proper names, "
                "abbreviations, and connectors (-, +, /, ->) to pack relationships. Across the "
                "bullets, cover in THIS PRIORITY whatever the sources support:\n"
                "   (a) license & type/availability (e.g. 'Open-source / MIT-licensed MoE');\n"
                "   (b) architecture & named techniques — name them EXACTLY (e.g. 'hybrid "
                "Mamba-Attention', 'two-tower MoE', 'LoRA rank 32');\n"
                "   (c) mechanism -> benefit (e.g. 'LatentMoE - fewer hidden dims, more routed "
                "experts -> better accuracy/param');\n"
                "   (d) headline spec with EXPLICIT comparison AND tradeoff (e.g. '~6x throughput "
                "vs comparable open LLMs at on-par accuracy');\n"
                "   (e) base model / dependencies / config (e.g. 'Built on <base>, LoRA rank 32');\n"
                "   (f) caveats / nuances (e.g. 'local & provider-agnostic, but LLM calls go to "
                "the chosen LLM').\n"
                "   Mark the single most important phrase/number per bullet with **double "
                "asterisks** (one span per bullet).\n"
                ...
                "STYLE — match this in-house analyst shorthand (ILLUSTRATIVE placeholders; use the "
                "real, sourced details for the actual topic, NOT these names):\n"
                "Title: AcmeAI Releases Vega-220B-A22B\n"
                "- Open-source MoE, hybrid SSM-Attention, **512K** context\n"
                "- **~5x** inference throughput vs comparable open LLMs, on-par accuracy\n"
                "- LatentRouter - fewer hidden dims, more routed experts -> better accuracy/param\n"
                "- Built on Vega-Base, **LoRA rank 32**; 4-bit weights fit smaller GPUs\n"
                "- 3 reasoning modes (Off/Reg/Med): Med = **2.5x fewer tokens** at ~7% drop\n\n"
                "OUTPUT RULES:\n"
                "- Return ONLY valid JSON matching the schema below.\n"
                "- No markdown fencing, no commentary outside JSON.\n"
                "- Newlines inside string values MUST be written as \\n.\n"
    ...
    return [
        {
            ...
            "parts": (
                ...
                "- KEEP terse technical shorthand — named architectures/techniques and "
                "abbreviations (MoE, LoRA, MTP, etc.) and exact specs are NOT 'vague' when "
                "supported by the SOURCES; do not delete them for being terse or jargon-heavy.\n"
